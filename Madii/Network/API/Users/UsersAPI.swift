...
struct LoginAPI {
    func loginWithEmail(email: String, password: String) -> APIEndpoint<PostLoginResponse> {
        let path = APIPaths.users.rawValue + "login/normal"
        
        let encodedPassword = EncodingService().encodingPassword(password)
...
struct LoginRepository {
    func loginWithEmail(email: String, password: String) {
        let endpoint = AuthAPI().login.loginWithEmail(email: email, password: password)
