...
class AchievementsAPI {
    ...
    func postJoySatisfaction(date: Date, achievementId: Int, satisfacton: String?, completion: @escaping (_ isSuccess: Bool) -> Void) {
        let url = "https://\(Bundle.main.infoDictionary?["BASE_URL"] ?? "nil baseUrl")/v2/achievements/finish"
        let headers: HTTPHeaders = [
            "Content-Type": "application/json",
            "Authorization": "Bearer \(keychain.get("accessToken") ?? "")"
        ]
